...
export default function Desayuno({ navigation }) {
    ...
    const traducirTexto = async (texto, source = 'en', target = 'es') => {
        try {
            ...
            if (data && data.translatedText) {
                ...
                console.warn("Respuesta inesperada de LibreTranslate:", data);
                return texto;
            }
        } catch (error) {
    ...
    const buscarPaisYRecetas = async () => {
        ...
        try {
            ...
            for (const receta of recetasEncontradas) {
                const instruccionesTraducidas = await traducirTexto(receta.strInstructions, 'en', 'es');
                recetasTraducidas.push({
                    ...receta,
